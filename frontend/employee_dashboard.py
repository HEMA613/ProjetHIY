...
class EmployeeDashboard:
    ...
    def _build(self):
        ...
        self.tab_home = tk.Frame(nb, bg=BG); nb.add(self.tab_home, text="  🏠  Tableau de bord  ")  # Onglet Accueil
        self.tab_req  = tk.Frame(nb, bg=BG); nb.add(self.tab_req,  text="  ➕  Nouvelle demande  ")  # Onglet Nouvelle demande
    ...
    def _build_home(self):
        ...
        my_vacs   = self.service.get_user_vacations(self.user["username"])  # Demandes de l'utilisateur
        pending   = len([v for v in my_vacs if v["status"] == "pending"])  # Nombre en attente
        approved  = len([v for v in my_vacs if v["status"] == "approved"])  # Nombre approuvées
        ...
        for count, label, color, emoji in [
            ...
            _label(c, label,        9, False, MUTED,  PANEL, "center").pack(pady=(0, 4))  # Label

        # Barre de progression pour l'utilisation des jours
        pct       = max(0.0, min(1.0, used / total)) if total else 0  # Pourcentage utilisé
        bar_color = SUCCESS if pct < 0.6 else WARNING if pct < 0.85 else DANGER  # Couleur selon le pourcentage

        prog_card = tk.Frame(self.tab_home, bg=PANEL)  # Carte pour la barre de progression
        prog_card.pack(fill="x", padx=26, pady=10, ipady=8)
        top_row = tk.Frame(prog_card, bg=PANEL)
        top_row.pack(fill="x", padx=16, pady=(10, 4))
        _label(top_row, f"Utilisation des jours de congé", 11, True, FG, PANEL).pack(side="left")  # Titre
        _label(top_row, f"{used} / {total} jours", 10, False, bar_color, PANEL).pack(side="right")  # Valeur
        ...
        # Liste des demandes
        _label(self.tab_home, "Mes demandes", 13, True, FG, BG).pack(anchor="w", padx=24, pady=(8, 4))  # Titre de la section
        ...
        if not my_vacs:  # Si aucune demande
            ...
            for v in reversed(my_vacs):  # Parcourt en ordre inverse (plus récentes en haut)
    ...
    def _req_card(self, parent, v):
        bg, fg, slabel = STATUS_CFG.get(v["status"], (PANEL, MUTED, v["status"]))  # Couleurs selon le statut
        try:
            ...
            period  = f"{s.strftime('%d %b')} → {e.strftime('%d %b %Y')}   ({days} jour{'s' if days > 1 else ''})"  # Période formatée
        except Exception:
            period = f"{v['start_date']} → {v['end_date']}"  # Format de secours

        card = tk.Frame(parent, bg=bg, highlightthickness=1, highlightbackground=BORDER)  # Frame de la carte
        ...
        _label(card, slabel, 10, True, fg, bg).pack(side="right", padx=16)  # Statut à droite
    ...
    def _build_request_form(self):
        ...
        form = tk.Frame(outer, bg=PANEL, highlightthickness=1, highlightbackground=BORDER)  # Frame du formulaire
        form.pack(fill="x")
        ...
        remaining = self.service.get_remaining_days(self.user["username"], total)  # Jours restants
        ...
        tk.Button(form, text="Envoyer la demande  →",
                  ...
                  cursor="hand2", command=self._submit).pack(padx=22, fill="x", ipady=9, pady=(0, 22))  # Bouton
    ...
    def _form_entry(self, parent):
        e = tk.Entry(parent, font=("Helvetica", 12),
                     ...
                     highlightcolor=ACCENT)  # Champ avec style
    ...
    def _render_calendar(self):
        ...
        _label(nav, f"{calendar.month_name[self.cal_month]}  {self.cal_year}",  # Titre du mois
        ...
        for week_i, week in enumerate(calendar.monthcalendar(self.cal_year, self.cal_month), 1):
            for day_i, day in enumerate(week):
                ...
                if marker:  # Si marqueur présent
                    _label(cell, marker, 10, False, num_fg, bg, "center").pack()  # Marqueur
        ...
        for color, txt in [
            ...
            _label(leg, txt, 9, False, MUTED, BG).pack(side="left", padx=(2, 16))  # Texte
