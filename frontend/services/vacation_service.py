...
class VacationService:
    ...
    def __init__(self):
        ...
        self.requests_file = os.path.join(backend_data, "demandes.json")    # Demandes de congés
        self.employees_file = os.path.join(backend_data, "employes.json")   # Données employés
        self.manager_file = os.path.join(backend_data, "manager.json")      # Données managers
    ...
    def _save_json(self, path, data):
        ...
        with open(path, "w", encoding="utf-8") as f:
            # Sauvegarde avec indentation et sans escaping ASCII (pour caractères spéciaux)
    ...
    def load_users(self):
        ...
        for e in employees:
            users.append({
                ...
                "total_days": e.get("vacation_balance", 25),  # Solde limité (défaut 25j)
            })
    ...
    def submit_request(self, username, start_date, end_date, reason=""):
        ...
        print(f"[DEBUG] utilisateurs disponibles: {[u['username'] for u in all_users]}")  # Debug temporaire
    ...
    def update_status(self, request_id, status):
        ...
        """
        status = status.lower() if isinstance(status, str) else status  # Normalisation minuscule
        demandes = self.get_all_vacations()
        updated = False
