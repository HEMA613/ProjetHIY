...
class ManagerDashboard:
    ...
    def _build(self):
        ...
        _label(hdr, "✦  Gestion des Congés  ·  Management", 13, True, FG, ACCENT).pack(
        ...
        nb.add(self.tab_req, text="  📋  Demandes  ")  # Onglet Demandes
        self.tab_cal = tk.Frame(nb, bg=BG)
        nb.add(self.tab_cal, text="  📅  Calendrier  ")  # Onglet Calendrier
        self.tab_emp = tk.Frame(nb, bg=BG)
        nb.add(self.tab_emp, text="  👥  Employees  ")  # Onglet Employés
    ...
    def _build_requests(self):
        ...
        )  # Étend horizontalement avec marges
        ...
        for count, label, color in [
            (
                len([v for v in vacs if v["status"] == "pending"]),
                "En attente",
                WARNING,
            ),  # Nombre en attente
            (
                len([v for v in vacs if v["status"] == "approved"]),
                "Approuvés",
                SUCCESS,
            ),  # Nombre approuvés
            (
                len([v for v in vacs if v["status"] == "rejected"]),
                "Refusés",
                DANGER,
            ),  # Nombre refusés
        ...
        _label(flt, "Filtrer :", 10, False, MUTED, BG).pack(
            side="left", padx=(0, 10)
        )  # Label "Filtrer :"
        ...
        style.map(
            "Dark.Treeview", background=[("selected", ACCENT)]
        ...
        self.tree = ttk.Treeview(
            frm, columns=cols, show="headings", style="Dark.Treeview", height=14
        )  # Tableau avec 14 lignes visibles
        ...
        self.tree.tag_configure(
            "pending", background="#451a03", foreground="#fde68a"
        )  # Couleur pour en attente
        self.tree.tag_configure(
            "approved", background="#052e16", foreground="#86efac"
        )  # Couleur pour approuvé
        self.tree.tag_configure(
            "rejected", background="#450a0a", foreground="#fca5a5"
        )  # Couleur pour refusé
        ...
        tk.Button(
            ...
            side="left", padx=12, ipadx=20, ipady=7
        )  # Bouton Approuver
        tk.Button(
            btn_row,
            text="❌   Refuser",
    ...
    def _refresh_req(self):
        ...
        users = {
            ...
            for u in self.service.load_users()
        }  # Dictionnaire des utilisateurs
        flt = self.filter_var.get()  # Récupère le filtre actuel
        for v in reversed(
            vacs
        ):  # Parcourt les demandes en ordre inverse (plus récentes en haut)
            if flt != "all" and v["status"] != flt:  # Applique le filtre
                continue
            requester = (
                v.get("employee_email") or v.get("username") or v.get("employee_name")
            )  # Identifiant du demandeur
            name = users.get(requester, requester)  # Nom complet ou identifiant
            days = VacationService.count_days(
                v["start_date"], v["end_date"]
            )  # Nombre de jours
            _, _, slabel = STATUS_CFG.get(
                v["status"], ("", "", v["status"])
            )  # Label du statut
            self.tree.insert(
                "",
                "end",
                iid=str(v["id"]),  # Insère la ligne dans le tableau
                values=(
                    name,
                    v["start_date"],
                    v["end_date"],
                    f"{days}j",
                    v.get("reason", "—"),
                    v.get("submitted_date", "—"),
                    slabel,
                ),
                tags=(v["status"],),
            )  # Applique le tag pour la couleur
    ...
    def _approve(self):
        ...
        if not sel:
            messagebox.showwarning(
                "Attention", "Sélectionnez une demande.", parent=self.root
            )
            return  # Avertissement si rien sélectionné
        self.service.update_status(int(sel[0]), "approved")  # Met à jour le statut
        messagebox.showinfo(
            "Succès", "Congé approuvé ✅", parent=self.root
        )  # Message de succès
        self._refresh_req()
        self._refresh_employees()  # Rafraîchit les vues
    ...
    def _reject(self):
        ...
        messagebox.showinfo(
            "Info", "Congé refusé ❌", parent=self.root
        )  # Message d'info
        self._refresh_req()
        self._refresh_employees()  # Rafraîchit les vues
    ...
    def _render_calendar(self):
        ...
        _label(
            ...
            f"{calendar.month_name[self.cal_month]}  {self.cal_year}",
            15,
            True,
            FG,
            BG,
        ...
        for week_i, week in enumerate(
            ...
            for day_i, day in enumerate(week):
                ...
                cell = tk.Frame(
                    grid, bg=bg, bd=0, highlightthickness=1, highlightbackground=BORDER
                )  # Frame pour la cellule
                ...
                if on_vac:  # Si des employés en congé
                    ...
                    _label(cell, names, 8, False, "#93c5fd", bg).pack(
                        anchor="w", padx=5
                    )  # Label des noms
    ...
    def _build_employees(self):
        self.emp_scroll_frame = tk.Frame(
            self.tab_emp, bg=BG
        )  # Frame pour le contenu défilable
    ...
    def _refresh_employees(self):
        for (
            ...
            self.emp_scroll_frame.winfo_children()
        ):  # Supprime tous les widgets existants
            w.destroy()

        _label(
            self.emp_scroll_frame,
            "Vue d'ensemble des employees",  # Titre de la section
            14,
            True,
            FG,
            BG,
        ...
        users = [
            u for u in self.service.load_users() if u["role"] == "employee"
        ]  # Liste des employés
        for u in users:  # Parcourt chaque employé
            ...
            pct = (
                max(0.0, min(1.0, used / total)) if total else 0
            )  # Pourcentage utilisé
            ...
            _label(top, u.get("full_name", u["username"]), 13, True, FG, PANEL).pack(
                side="left"
            )  # Nom de l'employé
            _label(
                top,
                f"{remaining} / {total} jours restants",
                10,
                False,
                bar_color,
                PANEL,
            ...
            rejected = len(
                [v for v in my_vacs if v["status"] == "rejected"]
            )  # Refusées
            _label(
                card,  # Label avec le résumé
                (
                    f"  ⌟ {pending} en attente   ✅ {approved} approuvés   "
                    f"❌ {rejected} refusés   |   {used} jours consommés"
                ),
                9,
                False,
                MUTED,
